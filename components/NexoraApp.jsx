...
import {
  ...
  RotateCcw, Sparkles, LogOut, Bell, Gift
} from "lucide-react";
...
const CHALLENGE_POOL = [
  ...
  "Acorde 30 minutos mais cedo amanhã e planeje o dia.",
  "Organize seu espaço de trabalho por 10 minutos.",
  "Faça 20 minutos de exercício físico.",
  "Leia 10 páginas de um livro.",
  "Elimine uma distração do seu celular por hoje.",
  "Escreva sua meta principal da semana num papel.",
  "Faça uma pausa de tela de 15 minutos e caminhe.",
  "Anote 3 coisas pelas quais você é grato hoje.",
  "Revise seus gastos do dia e anote onde pode economizar.",
  "Mande uma mensagem de apoio para alguém importante.",
  "Durma 30 minutos mais cedo hoje.",
  "Termine uma tarefa pendente há mais de 3 dias.",
  "Aprenda uma palavra nova em outro idioma.",
  "Passe 20 minutos sem usar nenhuma rede social.",
  "Escreva o que te impediu de evoluir ontem, e como evitar hoje.",
  "Planeje amanhã antes de dormir hoje.",
  "Faça uma ligação que você vem adiando.",
  "Beba água antes de cada refeição hoje.",
  "Desligue notificações desnecessárias do celular.",
  "Revise uma meta e ajuste o prazo se precisar.",
  "Medite ou respire fundo por 5 minutos.",
  "Escreva um resumo do seu dia antes de dormir.",
  "Evite açúcar em pelo menos uma refeição hoje.",
  "Organize sua lista de tarefas para amanhã.",
  "Compartilhe algo que você aprendeu recentemente.",
  "Faça uma tarefa que você tem mais medo de começar.",
  "Passe 1 hora sem verificar o celular.",
  "Escreva uma vitória pequena que teve essa semana.",
  "Ajude alguém sem esperar nada em troca.",
  "Revise seu progresso nas metas desta semana.",
  "Tire 10 minutos para arrumar sua cama e quarto.",
  "Evite reclamar por um dia inteiro.",
  "Escreva 3 metas para o próximo mês.",
  "Passe o dia sem adiar nenhuma tarefa pequena.",
  "Faça um agradecimento a alguém que te ajudou.",
];

function pickDailyChallenges(count = 8) {
  const d = new Date();
  const seed = Math.floor(d.getTime() / (1000 * 60 * 60 * 24));
  // embaralhamento determinístico baseado no dia, para gerar 8 desafios diferentes todos os dias
...
const MOTIVATIONAL_QUOTES = [
  ...
  "Disciplina é lembrar o que você quer, mesmo quando não tem vontade.",
  "Você não precisa ser perfeito, precisa ser consistente.",
  "O progresso de hoje é o resultado das escolhas de ontem.",
  "Não pare quando estiver cansado, pare quando terminar.",
  "Cada nível começou com alguém disposto a começar do zero.",
  "A procrastinação rouba mais sonhos do que o fracasso.",
  "Foco não é fazer mais, é fazer o que importa.",
  "Seu futuro está sendo construído nas ações de agora.",
  "Vencer a si mesmo hoje é a maior conquista do dia.",
  "Motivação te faz começar, hábito te faz continuar.",
  "Você está mais perto do que imagina — continue.",
  "A evolução não pede pressa, pede constância.",
  "Cada desafio concluído é um voto de confiança em você mesmo.",
  "Não conte os dias, faça os dias contarem.",
  "O esforço de hoje é invisível até virar resultado amanhã.",
  "Grandes metas se conquistam com passos pequenos e diários.",
  "Sua versão evoluída está do outro lado da disciplina.",
  "Feito é melhor que perfeito — comece.",
  "Você não precisa ver o topo da escada para dar o próximo passo.",
];
...
/* ---------------------------------------------------------
   Caixa Misteriosa Diária (recompensa variável)
--------------------------------------------------------- */

const BOX_REWARDS = [
  { min: 10, max: 25, weight: 55, label: "Bônus" },
  { min: 26, max: 50, weight: 28, label: "Ótimo bônus" },
  { min: 51, max: 90, weight: 13, label: "Bônus raro" },
  { min: 150, max: 150, weight: 4, label: "JACKPOT" },
];

function rollBoxReward() {
  const totalWeight = BOX_REWARDS.reduce((s, r) => s + r.weight, 0);
  let roll = Math.random() * totalWeight;
  for (const tier of BOX_REWARDS) {
    if (roll < tier.weight) {
      const amount = Math.floor(Math.random() * (tier.max - tier.min + 1)) + tier.min;
      return { amount, label: tier.label };
    }
    roll -= tier.weight;
  }
  return { amount: 10, label: "Bônus" };
}

function MysteryBox({ profile, onOpenBox }) {
  const [opening, setOpening] = useState(false);
  const [result, setResult] = useState(null);
  const openedToday = profile.last_box_date === todayStr();

  function handleOpen() {
    if (openedToday || opening) return;
    setOpening(true);
    const reward = rollBoxReward();
    setTimeout(() => {
      setResult(reward);
      setOpening(false);
      onOpenBox(reward.amount);
    }, 700);
  }
  ...
  return (
    <Card
      ...
      style={{
        background: result
          ? "linear-gradient(145deg,#1B1530,#0F1B24)"
          : "linear-gradient(145deg,#181822,#121218)",
        border: openedToday || result ? "1px solid #2A2A38" : "1px solid #F5B94255",
      }}
    >
      {result ? (
        <div className="flex items-center gap-3">
          <Gift size={26} color="#F5B942" />
          <div>
            <p className="text-[13px] font-semibold" style={{ color: "#F5B942" }}>{result.label}!</p>
            <p className="text-[12px]" style={{ color: "#8A8A9B" }}>Você ganhou +{result.amount} XP</p>
          </div>
        </div>
      ) : openedToday ? (
        <div className="flex items-center gap-3" style={{ opacity: 0.5 }}>
          <Gift size={22} color="#5C5C6E" />
          <div>
            <p className="text-[13px] font-medium">Caixa já aberta hoje</p>
            <p className="text-[12px]" style={{ color: "#8A8A9B" }}>Volte amanhã para uma nova surpresa.</p>
          </div>
        </div>
      ) : (
        <button onClick={handleOpen} className="w-full flex items-center gap-3">
          <Gift size={26} color="#F5B942" style={opening ? { animation: "nexora-shake .5s infinite" } : {}} />
          <div className="text-left">
            <p className="text-[14px] font-semibold">
              {opening ? "Abrindo..." : "Caixa Misteriosa disponível!"}
            </p>
            <p className="text-[12px]" style={{ color: "#8A8A9B" }}>
              {opening ? "..." : "Toque para revelar sua recompensa de hoje."}
            </p>
          </div>
        </button>
      )}
...
/* ---------------------------------------------------------
   Calendário de Sequência (Streak)
--------------------------------------------------------- */
...
function StreakCalendar({ userId, streak, goTo }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        ...
        .eq("user_id", userId)
        .gte("challenge_date", toDateKey(start));
      setCompletedDates(new Set((data || []).map((r) => r.challenge_date)));
      setLoading(false);
    }
    load();
  }, [userId]);
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      ...
      <div className="flex items-center gap-2 mb-6">
        <Flame size={22} color="#F5B942" />
        <h1 className="text-2xl font-semibold">Sequência</h1>
      </div>

      <Card className="p-5 mb-6 flex items-center gap-4" style={{ background: "linear-gradient(145deg,#1B1530,#0F1620)" }}>
        <Flame size={32} color="#F5B942" />
        <div>
          <p className="text-2xl font-semibold">{streak} <span className="text-sm font-normal" style={{ color: "#8A8A9B" }}>dias seguidos</span></p>
          <p className="text-[12px]" style={{ color: "#8A8A9B" }}>Não quebre o ritmo.</p>
        </div>
      </Card>
      ...
      {loading ? (
        ...
        <Card className="p-4">
          <div className="grid grid-cols-7 gap-1.5 mb-2">
            {weekLabels.map((l, i) => (
              <p key={i} className="text-[10px] text-center" style={{ color: "#5C5C6E" }}>{l}</p>
            ))}
          </div>
            ...
            {days.map((d, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: done ? "linear-gradient(135deg,#F5B942,#8B5CF6)" : "#181822",
                    border: isToday ? "1px solid #2DD4EE" : "1px solid #1F1F29",
                  }}
                >
                  {done && <Flame size={11} color="#08080D" />}
                </div>
              );
            })}
          </div>
      ...
      <p className="text-[12px] text-center mt-4" style={{ color: "#5C5C6E" }}>
        Complete pelo menos 1 desafio por dia para manter sua sequência viva.
      </p>
    </div>
  );
}

function Dashboard({ profile, todaysChallenges, completedToday, onCompleteChallenge, onOpenBox, goTo }) {
  ...
  const todayProgress = totalCount > 0 ? Math.round((doneCount / totalCount) * 100) : 0;

  const [showNotifs, setShowNotifs] = useState(false);

  const notifications = [];
  if (doneCount === 0) {
    notifications.push({ icon: Target, text: "Você tem desafios esperando por você hoje.", color: "#8B5CF6" });
  }
  if (doneCount === 0 && streak > 0) {
    notifications.push({ icon: Flame, text: "Seu streak está em risco. Complete pelo menos 1 desafio hoje.", color: "#F5B942" });
  }
  if (next) {
    const xpToNext = next.min - xp;
    if (xpToNext <= 50) {
      notifications.push({ icon: Star, text: `Você está a ${xpToNext} XP do próximo nível.`, color: "#2DD4EE" });
    }
  }
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      <div className="flex items-start justify-between mb-1">
        <p className="text-[13px]" style={{ color: "#8A8A9B" }}>Meu NEXORA</p>
        <div className="relative">
          <button onClick={() => setShowNotifs((s) => !s)} className="relative p-1">
            <Bell size={20} color="#8A8A9B" />
            {notifications.length > 0 && (
              <span
                className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold"
                style={{ background: "#F5B942", color: "#08080D" }}
              >
          ...
          {showNotifs && (
            <div
              className="absolute right-0 top-8 z-40 w-64 rounded-xl p-2 flex flex-col gap-1.5"
              style={{ background: "#121218", border: "1px solid #1F1F29" }}
            >
              {notifications.length === 0 ? (
                <p className="text-[12px] p-2" style={{ color: "#8A8A9B" }}>Nenhuma notificação por agora.</p>
              ) : (
                notifications.map((n, i) => {
                  const Icon = n.icon;
                  return (
                    <div key={i} className="flex items-start gap-2 p-2 rounded-lg" style={{ background: "#181822" }}>
                      <Icon size={14} color={n.color} className="shrink-0 mt-0.5" />
      ...
      <Card className="p-4 mb-6 flex items-start gap-3" style={{ background: "linear-gradient(145deg,#15121F,#0F1620)" }}>
        <Sparkles size={16} color="#F5B942" className="shrink-0 mt-0.5" />
        <p className="text-[13px] leading-snug italic" style={{ color: "#D4D4E0" }}>
          "{dailyPick(MOTIVATIONAL_QUOTES)}"
        </p>
      </Card>

      <MysteryBox profile={profile} onOpenBox={onOpenBox} />

      <div className="grid grid-cols-2 gap-3 mb-4">
        <button onClick={() => goTo("streak")} className="text-left">
          <Card className="p-4">
      ...
      <Card className="p-4 mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[13px] font-medium">{doneCount}/{totalCount} · {todayProgress}%</span>
        </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <p className="text-[15px] font-medium" style={{ color: "#8B5CF6" }}>DESAFIOS DE HOJE</p>
        <span className="text-[12px]" style={{ color: "#8A8A9B" }}>+20 XP cada</span>
      </div>
      <div className="flex flex-col gap-2 mb-6">
        {todaysChallenges.map((text, i) => {
          const done = completedToday.has(text);
          return (
            <Card key={i} className="p-3.5 flex items-center gap-3" style={done ? { opacity: 0.6 } : {}}>
              <button
                onClick={() => !done && onCompleteChallenge(text)}
                disabled={done}
                className="w-6 h-6 rounded-full flex items-center justify-center shrink-0"
                style={{
                  background: done ? "#4ADE80" : "transparent",
                  border: done ? "none" : "1px solid #2A2A38",
                }}
              >
                {done && <Check size={14} color="#08080D" />}
              </button>
              <p
                className="flex-1 text-[13px] leading-snug"
                style={{ color: done ? "#5C5C6E" : "#F4F4F8", textDecoration: done ? "line-through" : "none" }}
              >
                {text}
              </p>
            </Card>
...
const GOAL_SUGGESTIONS = [
  ...
  "Poupar dinheiro todo mês",
  "Ler um livro por mês",
  "Fazer exercício 3x por semana",
...
function Goals({ goals, userId, onGoalsChanged, onGoalCompleted, toast }) {
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      ...
      {showForm && (
        <Card className="p-4 mb-6">
          <p className="text-[11px] font-medium mb-2" style={{ color: "#8A8A9B" }}>Sugestões rápidas</p>
...
function WeeklySummary({ userId, streak, goTo }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: challenges }, { data: sessions }, { data: box }] = await Promise.all([
        supabase
          .from("daily_challenge_completions")
          .select("challenge_date")
          .eq("user_id", userId)
          .gte("challenge_date", weekAgoStr),
        supabase
          .from("focus_sessions")
      ...
      const activeDays = new Set((challenges || []).map((c) => c.challenge_date)).size;
      const focusMinutes = (sessions || []).reduce((s, r) => s + (r.duration_minutes || 0), 0);
      const focusSessionsCount = (sessions || []).length;
      const estimatedXp = challengeCount * 20 + focusSessionsCount * 20;

      setStats({ challengeCount, activeDays, focusMinutes, estimatedXp });
    }
    load();
  }, [userId]);
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      <div className="flex items-center gap-2 mb-1">
        <button onClick={() => goTo("profile")} className="text-[13px]" style={{ color: "#8A8A9B" }}>← Voltar</button>
      </div>
      <div className="flex items-center gap-2 mb-6">
        <TrendingUp size={22} color="#2DD4EE" />
        <h1 className="text-2xl font-semibold">Resumo Semanal</h1>
      </div>
        ...
        <>
          <Card className="p-5 mb-6" style={{ background: "linear-gradient(145deg,#15121F,#0F1620)" }}>
            <p className="text-[13px] leading-relaxed">
              Nos últimos 7 dias você esteve ativo em <strong>{stats.activeDays}</strong> dia(s), completou{" "}
              <strong>{stats.challengeCount}</strong> desafio(s) e ganhou aproximadamente{" "}
              <strong>{stats.estimatedXp} XP</strong>. {stats.activeDays >= 5 ? "Semana forte — continue assim!" : "Toda semana é uma nova chance de evoluir."}
            </p>
          </Card>

          <div className="grid grid-cols-2 gap-3 mb-3">
            <Card className="p-4">
              <p className="text-[11px] mb-1" style={{ color: "#8A8A9B" }}>Dias ativos</p>
              <p className="text-xl font-semibold">{stats.activeDays}<span className="text-sm font-normal" style={{ color: "#8A8A9B" }}>/7</span></p>
            </Card>
            <Card className="p-4">
              <p className="text-[11px] mb-1" style={{ color: "#8A8A9B" }}>Desafios concluídos</p>
              <p className="text-xl font-semibold">{stats.challengeCount}</p>
            </Card>
            <Card className="p-4">
              <p className="text-[11px] mb-1" style={{ color: "#8A8A9B" }}>Minutos de foco</p>
              <p className="text-xl font-semibold">{stats.focusMinutes}</p>
            </Card>
            <Card className="p-4">
              <p className="text-[11px] mb-1" style={{ color: "#8A8A9B" }}>XP ganho (estimado)</p>
              <p className="text-xl font-semibold">{stats.estimatedXp}</p>
            </Card>
          </div>

          <Card className="p-4 flex items-center gap-3">
            <Flame size={20} color="#F5B942" />
            <p className="text-[13px]">
              Sua sequência atual é de <strong>{streak}</strong> dia(s).
            </p>
          </Card>
        </>
      )}
...
function Profile({ profile, goals, goTo, onLogout, onUpdateProfile }) {
  ...
  async function saveName() {
    ...
    setSaving(true);
    await onUpdateProfile({ name: nameInput.trim() });
    setSaving(false);
    setEditing(false);
  }
  ...
  return (
    <div className="px-5 pt-6 pb-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                style={{ background: "linear-gradient(90deg,#8B5CF6,#2DD4EE)", color: "#08080D", opacity: saving ? 0.6 : 1 }}
              >
      ...
      <button onClick={() => goTo("weekly")} className="w-full mb-6">
        <Card className="p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <TrendingUp size={18} color="#2DD4EE" />
            <div className="text-left">
              <p className="text-[14px] font-medium">Resumo semanal</p>
              <p className="text-[12px]" style={{ color: "#8A8A9B" }}>Veja seu progresso dos últimos 7 dias.</p>
            </div>
          </div>
          <ChevronRight size={16} color="#8A8A9B" />
        </Card>
...
const STUDY_TASK_SUGGESTIONS = [
  "Revisar anotações",
  "Fazer exercícios",
  "Ler capítulo novo",
  "Assistir aula pendente",
  "Resumir matéria",
  "Estudar para prova",
];
...
function StudyPage({ userId, goTo }) {
  ...
  async function addTask(presetText) {
    const text = presetText || newTask;
    if (!text.trim()) return;
    const { data, error } = await supabase
      .from("study_tasks")
      .insert({ user_id: userId, title: text, done: false })
      .select()
      .single();
    if (!error && data) {
      setTasks((t) => [data, ...t]);
...
const BUSINESS_TASK_SUGGESTIONS = [
  "Contactar novo cliente",
  "Publicar produto",
  "Responder mensagens",
  "Fazer proposta",
  "Criar conteúdo",
...
function BusinessPage({ userId, goTo }) {
  ...
  return (
    <div className="px-5 pt-6 pb-6">
      ...
      <div className="flex gap-2 mb-3 flex-wrap">
        {BUSINESS_TASK_SUGGESTIONS.map((s) => (
          <button
            key={s}
            onClick={() => addTask(s)}
            className="text-[12px] px-3 py-1.5 rounded-full"
...
function ResetPassword({ onDone }) {
  ...
  return (
    ...
    >
      ...
      <h1 className="text-2xl font-semibold text-center mb-1">Criar nova senha</h1>
      ...
      {done ? (
        <div className="text-center">
          <p className="text-[14px] mb-6" style={{ color: "#4ADE80" }}>
...
export default function NexoraApp() {
  ...
  const [todaysChallenges] = useState(() => pickDailyChallenges(8));
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      setCompletedToday(new Set((completions || []).map((c) => c.challenge_text)));
      setProfile(prof);
      setGoals(goalsData || []);
      setScreen((s) => (s === "reset" ? s : "app"));
    }
  ...
  async function completeChallenge(text) {
    const { error } = await supabase.from("daily_challenge_completions").insert({
      user_id: session.user.id,
      challenge_date: todayStr(),
      challenge_text: text,
    });
    if (error) return; // já concluído ou erro de rede

    const isFirstToday = completedToday.size === 0;
    setCompletedToday((set) => new Set(set).add(text));

    await updateProfile({
      xp: profile.xp + 20,
      challenges_done: profile.challenges_done + 1,
      streak: isFirstToday ? profile.streak + 1 : profile.streak,
      last_challenge_date: todayStr(),
    });
    showToast("Desafio concluído · +20 XP");
  }
  ...
  async function onOpenBox(amount) {
    await updateProfile({ xp: profile.xp + amount, last_box_date: todayStr() });
  }
  ...
  if (screen === "landing") {
    return (
      <div style={{ maxWidth: 480, margin: "0 auto", fontFamily: "Inter, system-ui, sans-serif" }}>
        <style>{`@keyframes nexora-toast{from{opacity:0;transform:translate(-50%,8px)}to{opacity:1;transform:translate(-50%,0)}}@keyframes nexora-shake{0%,100%{transform:rotate(0deg)}25%{transform:rotate(-8deg)}75%{transform:rotate(8deg)}}`}</style>
  ...
  if (screen === "reset") {
    return (
      <div style={{ maxWidth: 480, margin: "0 auto", fontFamily: "Inter, system-ui, sans-serif" }}>
        <ResetPassword onDone={() => setScreen("app")} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {tab === "streak" && <StreakCalendar userId={session.user.id} streak={profile.streak} goTo={setTab} />}
      {tab === "business" && <BusinessPage userId={session.user.id} goTo={setTab} />}
      {tab === "rank" && <Ranking userId={session.user.id} />}
      {tab === "profile" && <Profile profile={profile} goals={goals} goTo={setTab} onLogout={handleLogout} onUpdateProfile={updateProfile} />}
      {tab === "weekly" && <WeeklySummary userId={session.user.id} streak={profile.streak} goTo={setTab} />}
      {tab === "achievements" && (
        <div>
