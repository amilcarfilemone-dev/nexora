...
function Dashboard({ profile, challengeStatus, onAcceptChallenge, onStartChallenge, onCompleteChallenge, goTo, challengeText }) {
  ...
  if (challengeStatus === "idle") {
    notifications.push({ icon: Target, text: "Você tem um desafio esperando por você.", color: "#8B5CF6" });
  }
  if (challengeStatus !== "done" && streak > 0) {
    notifications.push({ icon: Flame, text: "Seu streak está em risco. Complete o desafio de hoje.", color: "#F5B942" });
  }
  if (next) {
